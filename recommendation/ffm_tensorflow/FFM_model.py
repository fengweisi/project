    ...
    for i in range(input_shape):
        ...
        for j in range(i + 1, input_shape):
            ...
            tempValue = tf.reduce_sum(
                tf.multiply(weightLeftAfterCut, weightRightAfterCut))
...
if __name__ == '__main__':
    ...
    thirdWeight = createTwoDimensionWeight(
        input_x_size,  # 创建二次项的权重变量
        field_size,
    ...
    l2_norm = tf.reduce_sum(
        tf.add(
            ...
            tf.reduce_sum(tf.multiply(lambda_v, tf.pow(thirdWeight, 2)),
                          axis=[1, 2])))
    ...
    with tf.Session() as sess:
        ...
        for i in range(total_plan_train_steps):
            for t in range(all_data_size):
                ...
                print(
                    "After  {step} training   step(s)   ,   loss    on    training    batch   is  {predict_loss} "
